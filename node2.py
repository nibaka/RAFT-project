...
dict = {}
log_file = []
# order is : s1 - (connection_num1, ClientAddress_num1) - (connection_num2, ClientAddress_num2) - s3
my_connections = []

def leader_func(server_socket):
    print("start leader function...\n")
    connection_num1 = my_connections[1][0]
    ClientAddress_num1 = my_connections[1][1]
    connection_num2 = my_connections[2][0]
    ClientAddress_num2 = my_connections[2][1]

    leader_message = ("This is a message from LEADER with port:" + str(SERVER_ADDRESS[1][1])).encode('utf-8')
...
def follower_func(s1, s3, server_socket):
...
def leader_receive_client_message(client_connection, connection_num1, connection_num2):
    message = client_connection.recv(1024)
    message = pickle.loads(message)
    print(type(message))
    print(message)
...
def main():
    ...
    server_socket.bind(SERVER_ADDRESS[1])
    server_socket.listen(10)
    counter = 0
    random_number = numpy.random.random()
    dict[SERVER_ADDRESS[1][1]] = random_number

    s1 = socket.socket()
    s1.bind(("127.0.0.1", 5013))
    ...
    if current_state == STATES[0]:
        ...
        follower_func(s1, s3, server_socket)
